...
public class AppDifferenceSet64{

    int nAttributes;

    int nTuples;

    List<Subset> diffSet = new ArrayList<>();
...
//    }
    ...
    public List<Subset> generateDiffSet(List<List<Integer>> inversePli, String diffFp) {
        initiateDataStructure(inversePli);

        Map<BitSet, Long> diffSetMap = DataIO.readDiffSetsMap(diffFp);

        diffSet = Subset.bitSetMapToSubsets(nAttributes, diffSetMap);
